...
from bubbly_flows.tests.src.deterministic.classical_test import frst_symmetry_map, pick_peaks
from bubbly_flows.tests.src.common.bubble_sam3.backend import Sam3ConceptBackend
from bubbly_flows.tests.src.common.bubble_sam3.config import apply_cli_overrides, ensure_hf_home, load_config
from bubbly_flows.tests.src.common.bubble_sam3.outputs import (
    build_rgba_cutout,
    build_rgba_overlay,
    ensure_output_dir,
...
from bubbly_flows.tests.src.common.bubble_sam3.postprocess import (
    Instance,
...
from bubbly_flows.tests.src.common.bubble_sam3.tiling import create_tiles, pad_image
from bubbly_flows.tests.src.backends.frst_point_backend_fb import FrstPointBackendFB
from bubbly_flows.tests.src.backends.frst_point_backend_hf import FrstPointBackendHF
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--knn_k", type=int, default=3, help="Number of nearest-neighbor negative points per FRST center")
    parser.add_argument(
        "--hex_radius_factor",
        type=float,
        default=1.5,
        help="Negative-point hex radius as a multiple of r_max",
    )
    parser.add_argument(
        "--tile_size_factor",
        type=float,
        default=8.0,
        help="Tile size as a multiple of r_max",
    )
    parser.add_argument(
        "--tile_overlap_factor",
        type=float,
        default=2.5,
        help="Tile overlap as a multiple of r_max",
    )
    parser.add_argument(
        "--area_limit_factor",
        type=float,
        default=16.0,
        help="Per-mask area discard limit as factor * r_max^2",
    )
    ...
    parser.add_argument("--adaptive_area_min", type=int, default=20, help="Adaptive branch min area (px)")
    parser.add_argument("--adaptive_area_max", type=int, default=350, help="Adaptive branch max area (px)")
    parser.add_argument(
        "--adaptive_circularity_min",
        type=float,
        default=0.4,
        help="Adaptive branch min circularity",
    )
    parser.add_argument(
        "--adaptive_solidity_min",
        type=float,
        default=0.75,
        help="Adaptive branch min solidity",
    )
    parser.add_argument(
        "--adaptive_intensity_max",
        type=float,
        default=160.0,
        help="Adaptive branch max mean grayscale intensity",
    )
    parser.add_argument(
        "--label",
        default="bubble",
...
def process_single_image(
    ...
    # We calculate tiling
    tile_size = max(1, int(round(float(args.tile_size_factor) * float(args.r_max))))
    tile_overlap = max(0, int(round(float(args.tile_overlap_factor) * float(args.r_max))))
    ...
    area_limit = int(round(float(args.area_limit_factor) * float(args.r_max) * float(args.r_max)))
    total_discarded = 0
    total_masks = 0
    ...
    if not args.disable_candidates and frst_backend is not None:
        ...
        for idx, (x0, y0, x1, y1) in enumerate(tiles):
            ...
            obj_points, obj_labels = build_object_point_prompts(
                ...
                hex_radius=float(args.hex_radius_factor) * float(args.r_max),
            )
    ...
    if blackhat_enable:
        if blackhat_method == "adaptive":
            bh_area_min = args.adaptive_area_min
            bh_area_max = args.adaptive_area_max
            if args.blackhat_area_min is not None:
                bh_area_min = args.blackhat_area_min
            if args.blackhat_area_max is not None:
                bh_area_max = args.blackhat_area_max
            
            blackhat_instances = build_adaptive_instances(
                gray_raw,
                area_min=bh_area_min,
                area_max=bh_area_max,
                circularity_min=float(args.adaptive_circularity_min),
                solidity_min=float(args.adaptive_solidity_min),
                intensity_max=float(args.adaptive_intensity_max),
    ...
    if cfg["sam"].get("pcs_enable", True) and args.big_text_prompt:
        ...
        if args.big_backend == "hf":
            from bubbly_flows.tests.src.prompting.big_bubble_prompt_hf import run_big_prompt as run_big_prompt_hf
            big_instances = run_big_prompt_hf(image, args.big_text_prompt, cfg)
        else:
            from bubbly_flows.tests.src.prompting.big_bubble_prompt_fb import run_big_prompt as run_big_prompt_fb
